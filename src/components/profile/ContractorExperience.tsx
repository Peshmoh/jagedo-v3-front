...
// Can hold a string (URL) or a File object
...
const ContractorExperience = () => {
  ...
  useEffect(() => {
    const fetchExperience = async () => {
      const contractorTypeLabels = {
        ...
        'road-works': 'Road and other Civil Works'
      };

      const createInitialCategories = () => {
        if (user?.contractorTypes) {
          const contractorTypes = user.contractorTypes.split(',').filter(Boolean);
          return contractorTypes.map(type => ({
            id: Math.random().toString(),
            category: contractorTypeLabels[type] || type,
            categoryClass: "",
            yearsOfExperience: "",
        ...
        return [{ id: Math.random().toString(), category: "", categoryClass: "", yearsOfExperience: "", certificateFile: null, licenseFile: null, isEditing: true }];
      };
      ...
      try {
        ...
        console.error("Failed to fetch contractor experience:", error);
  ...
  const updateExperienceOnServer = async (
    ...
    const contractorCategories = await Promise.all(
      currentCategories.map(async (cat) => {
        const certificate = (cat.certificateFile instanceof File) ? (await uploadFile(cat.certificateFile)).url : cat.certificateFile;
        const license = (cat.licenseFile instanceof File) ? (await uploadFile(cat.licenseFile)).url : cat.licenseFile;
    ...
    const contractorProjects = await Promise.all(
      currentProjects.map(async (proj) => {
        const projectFile = (proj.projectFile instanceof File) ? (await uploadFile(proj.projectFile)).url : proj.projectFile;
        const referenceLetterUrl = (proj.referenceLetterFile instanceof File) ? (await uploadFile(proj.referenceLetterFile)).url : proj.referenceLetterFile;
  ...
  const removeProjectFile = async (id: string, field: 'projectFile' | 'referenceLetterFile') => {
    ...
    try {
      await toast.promise(
        updateExperienceOnServer(categories, updatedProjects),
        {
          loading: 'Deleting file...',
          success: 'File deleted successfully!',
          error: (err: any) => err.response?.data?.message || 'Failed to delete file.',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await toast.promise(
        updateExperienceOnServer(categories, projects),
        {
  ...
  const renderFileState = (file: FileOrUrl, onRemove: () => void, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void) => {
    ...
    if (isReadOnly) {
      return <span className="text-xs text-gray-500 p-2">No file provided.</span>;
    }
    return <input type="file" onChange={onChange} className={fileInputStyles} />;
  };

  if (isLoadingProfile) {
    return <div className="p-8 text-center text-gray-600">Loading contractor profile...</div>;
  }
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg p-4 md:p-8">
        {!submitted ? (
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            {isReadOnly && (
              <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg text-sm" role="alert">
                    ...
                    {categories.map((cat) => (
                      <tr key={cat.id} className="block md:table-row border-b-2 md:border-b last:border-b-0 border-gray-200 md:hover:bg-gray-100 p-4 md:p-0 mb-4 md:mb-0 rounded-lg md:rounded-none bg-white md:bg-transparent shadow-md md:shadow-none relative">
                        ...
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          ...
                          <select value={cat.categoryClass} onChange={e => handleCategoryChange(cat.id, 'categoryClass', e.target.value)} className={inputStyles} required disabled={isReadOnly}><option value="">Select Class</option>{["NCA 1", "NCA 2", "NCA 3", "NCA 4", "NCA 5", "NCA 6", "NCA 7", "NCA 8"].map(c => <option key={c} value={c}>{c}</option>)}</select>
                        </td>
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          <span className="font-semibold text-gray-600 md:hidden">Years of Exp.: </span>
                          <select value={cat.yearsOfExperience} onChange={e => handleCategoryChange(cat.id, 'yearsOfExperience', e.target.value)} className={inputStyles} required disabled={isReadOnly}><option value="">Select Years</option>{["10+ years", "5-10 years", "3-5 years", "1-3 years"].map(y => <option key={y} value={y}>{y}</option>)}</select>
                        </td>
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          <span className="font-semibold text-gray-600 md:hidden">Certificate: </span>
                        ...
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          <span className="font-semibold text-gray-600 md:hidden">License: </span>
                    ...
                    {projects.map((proj) => (
                      <tr key={proj.id} className="block md:table-row border-b-2 md:border-b last:border-b-0 border-gray-200 md:hover:bg-gray-100 p-4 md:p-0 mb-4 md:mb-0 rounded-lg md:rounded-none bg-white md:bg-transparent shadow-md md:shadow-none relative">
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          ...
                          <input type="text" placeholder="Enter project name" value={proj.projectName} onChange={e => handleProjectChange(proj.id, 'projectName', e.target.value)} className={inputStyles} required disabled={isReadOnly} />
                        </td>
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          <span className="font-semibold text-gray-600 md:hidden">Project File: </span>
                          {renderFileState(proj.projectFile, () => removeProjectFile(proj.id, 'projectFile'), e => handleProjectFileChange(proj.id, 'projectFile', e.target.files?.[0]))}
                        </td>
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 align-top">
                          <span className="font-semibold text-gray-600 md:hidden">Reference Letter: </span>
                          {renderFileState(proj.referenceLetterFile, () => removeProjectFile(proj.id, 'referenceLetterFile'), e => handleProjectFileChange(proj.id, 'referenceLetterFile', e.target.files?.[0]))}
                        </td>
                        <td className="block md:table-cell py-2 md:py-3 md:px-3 md:text-center align-top absolute top-2 right-2 md:relative md:top-auto md:right-auto">
