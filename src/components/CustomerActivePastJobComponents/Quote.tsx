...
const ProjectQuoteForm = () => {
    ...
    useEffect(() => {
        if (jobData) {
            // If assignedBid exists, use its detailed data
            if (jobData.assignedBid) {
                const {
                ...
                // Add professional fees to summary if it's greater than 0
                if (professionalFeeSubtotal > 0) {
                    summary.push({
                        id: summary.length + 1,
                        title: "Service provider Fees",
                        amount: professionalFeeSubtotal,
                        details: professionalFees.map((fee, index) => ({
                            no: index + 1,
                            description: `${fee.fee} (${fee.qty} ${fee.uom} @ ${formatCurrency(fee.rate)})`,
                            amount: fee.amount,
                        })),
                    });
                }

                // Add expenses to summary if it's greater than 0
                ...
                // Add Jagedo fee to summary if it's greater than 0
                if (jagedoCommission > 0) {
                    summary.push({
                        id: summary.length + 1,
                        title: "Jagedo Service Fee",
                        amount: jagedoCommission,
                        details: [{ no: 1, description: "Platform and management fee.", amount: jagedoCommission }],
                    });
                }
                ...
                // Fallback to original data if assignedBid is not available
                const summary = [
                    {
                        id: 1,
                ...
                ].filter(bill => bill.amount > 0); // Also filter here for consistency
