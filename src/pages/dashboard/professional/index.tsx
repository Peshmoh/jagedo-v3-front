...
export default function ProfessionalDashboard() {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            setIsLoading(true);
            setError(null); // --- FIX: Clear previous errors on new fetch ---
            try {
                ...
                if (ordersResponse.success) {
                    ...
                    setError(prev => prev ? `${prev} And could not fetch order requests.` : "Could not fetch order requests.");
                }
            } catch (err) {
    ...
    const getJobsByStatus = (status: string): JobRequest[] => {
        ...
        switch (status) {
            case "new":
                return jobRequests.filter((job) => ["new", "request"].includes(job.status.toLowerCase())).sort(sortByDateDesc);
            case "draft":
                return jobRequests.filter((job) => job.status.toLowerCase() === "draft").sort(sortByDateDesc);
            case "bid":
                return jobRequests.filter((job) => ["bid", "quotation", "submitted"].includes(job.status.toLowerCase())).sort(sortByDateDesc);
            case "active":
    ...
    const getStatusColor = (status: string) => {
        ...
        switch (lowerStatus) {
            ...
            case "bid": return "bg-indigo-100 text-indigo-800";
                ...
                {jobsToDisplay.length > 0 ? (
                    jobsToDisplay.map((job) => (
                        <JobOrderCard
                            ...
                            onViewDetail={(selectedJob) => navigate(`/professional/${activeSection}/job/${selectedJob.id}`)}
                        />
                    ))
                ) : (
                ...
                {ordersToDisplay.length > 0 ? (
                    ordersToDisplay.map((order) => (
                        <JobOrderCard
                            ...
                            getStatusColor={getStatusColor}
                            getStatusIcon={getStatusIcon}
                            onViewDetail={(selectedOrder) => navigate(`/professional/${activeSection}/order/${selectedOrder.id}`)}
                        />
                    ))
                ) : (
                    <div className="text-center py-8 text-gray-500">
    ...
    const renderSectionContent = () => {
        if (isLoading) {
            return <div className="flex justify-center items-center p-16"><Loader className="animate-spin h-8 w-8 text-[#00007a]" /></div>;
        }
        if (error) {
            return <p className="text-center p-8 text-red-600">{error}</p>;
        }

        // --- CRITICAL FIX: Declared variables here to ensure they are in scope. ---
        // --- REFACTOR: Simplified logic to handle the 'bid' section and others consistently. ---
        let sectionTitle = "";
        let sectionDescription = "";

        const currentJobs = getJobsByStatus(activeSection);
        const currentOrders = getOrdersByStatus(activeSection);
        const jobBadgeCount = currentJobs.length;
        const orderBadgeCount = currentOrders.length;

        if (activeSection === "bid") {
            sectionTitle = "Your Bids";
            sectionDescription = "Jobs and material orders you have bid on";
        } else {
            ...
            switch (activeSection) {
                ...
                case "draft": sectionDescription = "Draft jobs and orders you are working on"; break;
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center justify-between animate-fade-in">
                    ...
                    {/* --- FIX: Simplified and consistent badge logic --- */}
                    <div className="flex gap-2">
                        <Badge className="bg-[#00a63e] text-white px-3 py-1">
                            {jobBadgeCount} {jobBadgeCount === 1 ? 'Job' : 'Jobs'}
                        </Badge>
                        <Badge className="bg-[#00007a] text-white px-3 py-1">
                            {orderBadgeCount} {orderBadgeCount === 1 ? 'Order' : 'Orders'}
                        </Badge>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-end">
                    <div className="relative flex-1 max-w-sm w-full">
                        ...
                        <Input
                            // --- FIX: Updated placeholder to be more generic ---
                ...
                <div className="grid gap-6 max-w-6xl mx-auto">
                    {/* --- CRITICAL FIX: `currentJobs` and `currentOrders` are now correctly passed --- */}
