    ...
    return (
        <div>
            ...
            <div className="hidden md:block">
                {/* MODIFICATION: Added 'table-fixed' for predictable column widths */}
                <table className="w-full border-collapse rounded-lg overflow-hidden table-fixed">
                    <thead>
                        <tr className="bg-gray-50">
                            <th className="px-4 py-3 text-left text-sm font-semibold text-gray-800 w-4/5">File Name</th>
                    ...
                    <tbody>
                        {files.map((fileUrl, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                {/* MODIFICATION: Simplified and applied truncate directly to the table cell */}
                                <td className="px-1 py-3 border-t border-gray-200 text-sm truncate" title={getFileNameFromUrl(fileUrl)}>
                                    {getFileNameFromUrl(fileUrl)}
                                </td>
                                <td className="pr-1 py-3 border-t border-gray-200">
                ...
                {files.map((fileUrl, index) => (
                    <div key={index} className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                        <div className="flex justify-between items-center space-x-2">
                            {/* MODIFICATION: Added 'min-w-0' to allow truncation in flexbox */}
                            <span className="text-sm font-medium text-gray-800 truncate min-w-0" title={getFileNameFromUrl(fileUrl)}>
                                {getFileNameFromUrl(fileUrl)}
                            </span>
...
function JobProffRequestDetails() {
    ...
    const handleClick = () => {
        navigate(`/professional/bid/job/${id}`);
    };
                    ...
                    <div className="flex flex-col lg:flex-row lg:justify-between gap-4">
                        ...
                        <div className="w-full lg:w-1/2 lg:pl-8 lg:border-l border-gray-200 space-y-4 mt-6 lg:mt-0">
                            {jobRequest.payments.length > 0 ? (
                                <a href="#" onClick={() => navigate(`/receipt/${jobRequest.payments[jobRequest.payments.length - 1].id}`)} className="flex items-center space-x-2 bg-green-50 p-4 rounded-lg border border-green-200 hover:bg-green-100 transition-colors">
                            ...
                            {jobRequest.managedBy.toLowerCase() === 'jagedo' && (
                                <div className="bg-blue-100 p-4 rounded-2xl shadow-md border border-gray-200">
                            ...
                            {jobRequest.managedBy.toLocaleLowerCase() == 'self' && (
                                <div className="bg-blue-100 p-4 rounded-2xl shadow-md border border-gray-200">
                                    <h3 className="text-xl font-bold text-blue-900 mb-2">Self Managed</h3>
                                    <p className="text-lg font-semibold text-gray-800 mb-4">You Oversee</p>
                                    <ul className="space-y-3 text-gray-700">
                                        <li className="flex items-start"><TiTick className="text-green-500 mr-2 text-xl flex-shrink-0" /><span>Arrival time</span></li>
                                        <li className="flex items-start"><TiTick className="text-green-500 mr-2 text-xl flex-shrink-0" /><span>Scope budget</span></li>
                                        <li className="flex items-start"><TiTick className="text-green-500 mr-2 text-xl flex-shrink-0" /><span>Quality through peer review & professionalism</span></li>
